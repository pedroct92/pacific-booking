...
public interface SiteAvailabilityRepository extends CrudRepository<SiteAvailability, Long> {
    ...
    @Query("select a from SiteAvailability as a join Site as s on a.site.id = s.id where a.remainingPlaces > 0 " +
            " and a.dayDate >= :startDate and a.dayDate <= :endDate and a.site.id = :siteId")
